...
dice_stats get_dice_stat_from_str(char *line)
{
    dice_stats stats;// 10+10d2
    stats.base = 0;
    stats.dice = 0;
    stats.sides = 0;
    int i;
    ...
    for(i = 0; i <= (int)strlen(line); i++)
    {
        if(line[i] == '+')
        {
            stats.base = atoi(substr(line, st, en));
            st = en;
            en++;
        }
        else if(line[i] == 'd')
        {
            stats.dice = atoi(substr(line, st, en));
            st = en + 1;
            en++;
        }
        else if(i == (int)strlen(line))
        {
            stats.sides = atoi(substr(line, st, en));
            return stats;
        }
        else
        {
...
int get_num_desc(const char *object,const char *file)
{
    const char *home;
    char *filename;
    size_t len;
    ...
    len = (strlen(home) + strlen(SAVE_DIR) + strlen(file) + 3);
...
void read_monster_desc_to_dungeon(dungeon *d) {
    ...
    char *filename;
    size_t len;
    d->num_mons_desc = get_num_desc("BEGIN MONSTER\n", MONSTER_DESC_FILE);
    d->md = new monster_description[d->num_mons_desc];
    if (!(home = getenv("HOME"))) {
        fprintf(stderr, "\"HOME\" is undefined. Using working directory.\n");
        home = ".";
    }
    len = (strlen(home) + strlen(SAVE_DIR) + strlen(MONSTER_DESC_FILE) + 3);

    filename = (char *) malloc(len * sizeof(*filename));
    sprintf(filename, "%s/%s/", home, SAVE_DIR);
    makedirectory(filename);
    strcat(filename, MONSTER_DESC_FILE);

    FILE *f = fopen(filename, "r");
    if (f == NULL) {
        printf("Error reading %s", MONSTER_DESC_FILE);
        exit(1);
    }
    char metadata[34];
    fgets(metadata, sizeof metadata, f);
    if (!strcmp(metadata, "RLG327 MONSTER DESCRIPTION 1\n")) {
        char line[512];
        ...
        {
            //printf("%s\n", line);
            if (!strcmp(substr(line, 0, 5), "NAME ")) {
                d->md[num].name = substr(line, 5, (int)strlen(line) - 1);
            }
            if (!strcmp(line, "DESC\n")) {

                while (fgets(line, sizeof line, f) != NULL && (int)strcmp(line, ".\n")) {
                    strcat(description, line);
                }
                printf("%s\n", description);
                strcpy(d->md[num].desc, description);
                description[0] = '\0';
            }
            if (!strcmp(substr(line, 0, 6), "COLOR ")) {
                d->md[num].colors = substr(line, 6, (int)strlen(line) - 1);
            }
            if (!strcmp(substr(line, 0, 6), "SPEED ")) {
                d->md[num].speed = get_dice_stat_from_str(substr(line, 6, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 5), "ABIL ")) {
                for(i = 5; i < (int)strlen(line) - 1; i++)
                {
                    if(line[i] == ' ' || line[i] == '\n')
                    {
                        temp = substr(line, st, i);
                        if(!strcmp(temp, "SMART"))
                        {
                            d->md[num].ability += NPC_SMART;
                        }
                        if(!strcmp(temp, "TELE"))
                        {
                            d->md[num].ability += NPC_TELEPATH;
                        }
                        if(!strcmp(temp, "TUNNEL"))
                        {
                            d->md[num].ability += NPC_TUNNEL;
                        }
                        if(!strcmp(temp, "ERRATIC"))
                        {
                            d->md[num].ability += NPC_ERRATIC;
                        }
                        if(!strcmp(temp, "PASS"))
                        {
                            d->md[num].ability += NPC_PASS;
                        }
                        if(!strcmp(temp, "BOSS"))
                        {
                            d->md[num].ability += NPC_BOSS;
                        }
                        if(!strcmp(temp, "DESTROY"))
                        {
                            d->md[num].ability += NPC_DESTROY;
                        }
                        if(!strcmp(temp, "UNIQ"))
                        {
                            d->md[num].ability += NPC_UNIQ;
                        }
                        if(!strcmp(temp, "PICKUP"))
                        {
                            d->md[num].ability += NPC_PICKUP;
                        }
                    }
                }
            }
            if (!strcmp(substr(line, 0, 3), "HP ")) {
                d->md[num].hp = get_dice_stat_from_str(substr(line, 3, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "DAM ")) {
                d->md[num].attack_damage = get_dice_stat_from_str(substr(line, 4, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 5), "SYMB ")) {
                d->md[num].symbol = substr(line, 5, 6)[0];
            }
            if (!strcmp(substr(line, 0, 5), "RRTY ")) {
                d->md[num].rarity = atoi(substr(line, 5, (int)strlen(line) - 1));
            }
            if (!strcmp(line, "END\n")) {
                num++;
            }
...
void read_objects_desc_to_dungeon(dungeon *d) {
    ...
    char *filename;
    size_t len;
    d->num_objs_desc = get_num_desc("BEGIN OBJECT\n", OBJECTS_DESC_FILE);
    d->od = new object_description[d->num_objs_desc];
    if (!(home = getenv("HOME"))) {
        fprintf(stderr, "\"HOME\" is undefined. Using working directory.\n");
        home = ".";
    }
    len = (strlen(home) + strlen(SAVE_DIR) + strlen(OBJECTS_DESC_FILE) + 3);

    filename = (char *) malloc(len * sizeof(*filename));
    sprintf(filename, "%s/%s/", home, SAVE_DIR);
    makedirectory(filename);
    strcat(filename, OBJECTS_DESC_FILE);

    FILE *f = fopen(filename, "r");
    if (f == NULL) {
        printf("Error reading %s", OBJECTS_DESC_FILE);
        exit(1);
    }
    char metadata[34];
    fgets(metadata, sizeof metadata, f);
    if (!strcmp(metadata, "RLG327 OBJECT DESCRIPTION 1\n")) {
        char line[512];
        char *description = new char[1024];
        int num = 0;
        ...
        {
            if (!strcmp(substr(line, 0, 5), "NAME ")) {
                d->od[num].name = substr(line, 5, (int)strlen(line) - 1);
            }
            if (!strcmp(line, "DESC\n")) {
                ...
                printf("%s\n", description);
                strcpy(d->od[num].desc, description);
                description[0] = '\0';
            }
            if (!strcmp(substr(line, 0, 6), "COLOR ")) {
                d->od[num].colors = substr(line, 6, (int)strlen(line) - 1);
            }
            if (!strcmp(substr(line, 0, 5), "TYPE ")) {
                d->od[num].type = substr(line, 5, (int)strlen(line) - 1);
            }
            if (!strcmp(substr(line, 0, 6), "SPEED ")) {
                d->od[num].speed = get_dice_stat_from_str(substr(line, 6, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "HIT ")) {
                d->od[num].hit = get_dice_stat_from_str(substr(line, 4, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "DAM ")) {
                d->od[num].dam = get_dice_stat_from_str(substr(line, 4, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 5), "ATTR ")) {
                d->od[num].attr = get_dice_stat_from_str(substr(line, 5, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "VAL ")) {
                d->od[num].val = get_dice_stat_from_str(substr(line, 4, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 7), "WEIGHT ")) {
                d->od[num].weight = get_dice_stat_from_str(substr(line, 7, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 6), "DODGE ")) {
                d->od[num].dodge = get_dice_stat_from_str(substr(line, 6, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "DEF ")) {
                d->od[num].def = get_dice_stat_from_str(substr(line, 4, (int)strlen(line) - 1));
            }
            if (!strcmp(substr(line, 0, 4), "ART ")) {
                d->od[num].art = !strcmp(substr(line, 4, strlen(line) - 1), "TRUE");
            }
            if (!strcmp(substr(line, 0, 5), "RRTY ")) {
                d->od[num].rarity = atoi(substr(line, 5, (int)strlen(line) - 1));
            }
            if (!strcmp(line, "END\n")) {
                num++;
            }
