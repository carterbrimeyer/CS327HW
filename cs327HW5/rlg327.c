...
int main(int argc, char *argv[]) {
    ...
    * and don't write to disk.                                      */
    ...
    config_pc(&d);
    gen_monsters(&d);
    init_render_terminal();
    update_ui(&d);
    d.exit = false;
    while (!d.exit && pc_is_alive(&d)) {
        render_dungeon(&d);
        refresh();
        do_moves(&d);
    ...
    mvprintw(21, 1, "%s", pc_is_alive(&d) ? victory : tombstone);
    //mvprintw(21, 1, ("You defended your life in the face of %d deadly beasts.\nYou avenged the cruel and untimely murders of %d peaceful dungeon residents.\n", d.pc.kills[kill_direct], d.pc.kills[kill_avenged]));
