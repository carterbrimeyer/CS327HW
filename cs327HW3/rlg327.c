...
void render_path_dungeon(dungeon_t *d, corridor_path_t path[DUNGEON_Y][DUNGEON_X])
{
    pair_t p;
            ...
            if (d->pc[dim_x] == p[dim_x] && d->pc[dim_y] == p[dim_y]) {
                ...
                switch (mappair(p)) {
                    ...
                    case ter_wall_immutable:
                        ...
                        break;
                    default:
                        printf("%d", path[p[dim_y]][p[dim_x]].cost % 10);
                        break;
                }
            }
        }
...
void render_tunnel_dungeon(dungeon_t *d, corridor_path_t path[DUNGEON_Y][DUNGEON_X], corridor_path_t *o)
{
    pair_t p;
    ...
    for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
            if (d->pc[dim_x] == p[dim_x] && d->pc[dim_y] == p[dim_y]) {
                printf("@");
            } else if(mappair(p) == ter_wall_immutable)
            {
                printf(" ");
            }
            else {
    ...
    //int hardnessn = d->hardness[p->pos[dim_y]][p->pos[dim_x]] / 85
    for (i = 0; i < DUNGEON_Y; i++) {
        for (j = 0; j < DUNGEON_X; j++) {
            ...
            else if(i == d->pc[dim_y] && j == d->pc[dim_x])
            {
                printf("@");
            }
            else
            {
                printf("%d", 1 + d->hardness[i][j] / 85);
            }
...
static void dijkstra_pathfinding_tunnels(dungeon_t *d)
{
    static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
    static uint32_t initialized = 0;
    heap_t h;
    ...
    while ((p = heap_remove_min(&h))) {
        ...
        if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
            (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > p->cost + 1 + hardnessn)){
            path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost = p->cost + 1 + hardnessn;
            heap_decrease_key_no_replace(&h, path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn);
        }
        if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
            (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > p->cost + 1 + hardnessn)) {
            path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost = p->cost + 1 + hardnessn;
            heap_decrease_key_no_replace(&h, path[p->pos[dim_y]    ]
            [p->pos[dim_x] - 1].hn);
        }
        ...
        if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
            (path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost > p->cost + 1 + hardnessn)) {
            path[p->pos[dim_y]  - 1 ][p->pos[dim_x] - 1].cost = p->cost + 1 + hardnessn;
            heap_decrease_key_no_replace(&h, path[p->pos[dim_y] - 1]
            [p->pos[dim_x] - 1].hn);
        }
    ...
    render_tunnel_dungeon(d, path, p);
}
static void dijkstra_pathfinding(dungeon_t *d)
{
    static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
    static uint32_t initialized = 0;
    heap_t h;
...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  dijkstra_pathfinding(&d);
  dijkstra_pathfinding_tunnels(&d);
  delete_dungeon(&d);
