...
void pc_observe_terrain(pc *p, dungeon *d)
{
  ...
  /* Take one off the x range because we already hit the corners above. */
...
int pc_pickup_obj(pc *p, object *obj)
{
    for(int i = 0; i < PC_MAX_INV; i++)
    {
        ...
        {
            io_queue_message("You picked up the %s", obj->get_name());
            p->inventory[i] = obj;
            return 1;
        }
    }
    io_queue_message("No space to pick up the %s", obj->get_name());
    return 0;
}
int pc_equip_obj(pc *p, int slot)
{
    if(p->inventory[slot] != NULL) {
        int num;
        switch (p->inventory[slot]->get_type()) {
            case objtype_WEAPON:
                num = SLOT_WEAPON;
                p->damage->set((p->inventory[slot]->get_damage_base()),(p->inventory[slot]->get_damage_number()), (p->inventory[slot]->get_damage_sides()));
                break;
            ...
            case objtype_AMULET:
                ...
                break;
            case objtype_RING:
                if(p->equipment[SLOT_RING_RH] == NULL)
                {
                    num = SLOT_RING_RH;
                }
                else
                ...
                break;
            default:
                io_queue_message("Cannot equip %s", p->inventory[slot]->get_name());
                return 0;
        }
        object *hold_this = NULL;
        hold_this = p->equipment[num];
        p->equipment[num] = p->inventory[slot];
        p->inventory[slot] = hold_this;

        io_queue_message("You equipped %s", p->equipment[num]->get_name());
        return 1;
    }
    io_queue_message("Nothing to equip at %d!", slot);
    return -1;
}
int pc_drop_obj(dungeon *d, int num)
{
    ...
    {
        io_queue_message("You dropped %s", d->PC->inventory[num]->get_name());
        d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] = d->PC->inventory[num];
        d->PC->inventory[num] = NULL;

        return 1;
    }
    io_queue_message("Nothing to drop at %d", num);
    return 0;
}
int pc_dequip(dungeon *d, int num)
{
    if(d->PC->equipment[num] != NULL) {
        for (int i = 0; i < PC_MAX_INV; i++) {
            if(d->PC->inventory[i] == NULL)
            {
                io_queue_message("You removed %s", d->PC->equipment[num]->get_name());
                d->PC->inventory[i] = d->PC->equipment[num];
                d->PC->equipment[num] = NULL;
                return 1;
            }
        }
    }
    io_queue_message("No where to put %d", num);
    return 0;
}