    ...
    do {
        ...
        switch (key = getch()) {
            ...
            case 'e':
                ...
                break;
            case 'w':
                num = promptUserForInvSlot(d);
                if(num > -1 && num < 10)
                {
                    pc_equip_obj(d->PC, num);
                }
                fail_code = 1;
                break;
            case 'd':
            ...
            case 'x':
                ...
                if(num > -1 && num < 10)
                {
                    io_queue_message("%s has been removed", d->PC->inventory[num]);
...
void io_show_inv(dungeon *d) {
    WINDOW *invWin = newwin(16, 40, 1, 20);
    wclear(invWin);
    box(invWin, '|', '-');
    mvwprintw(invWin, 1, 15, "Inventory");
    for (int i = 0; i < PC_MAX_INV; i++) {
        if (d->PC->inventory[i]) {
            mvwprintw(invWin, 2 + i, 4, "%d. %s", i, d->PC->inventory[i]->get_name());
        } else {
            mvwprintw(invWin, 2 + i, 4, "~~Empty~~");
        }
    }
    mvwprintw(invWin, 14, 12, "ESC to exit...");
    wrefresh(invWin);
...
void io_show_eqp(dungeon *d) {
    WINDOW *eqpWin = newwin(20, 40, 1, 20);
    wclear(eqpWin);
    box(eqpWin, '|', '-');
    mvwprintw(eqpWin, 1, 15, "Equipment");
    mvwprintw(eqpWin, 2, 6, "Weapon: ");
    mvwprintw(eqpWin, 3, 5, "Offhand: ");
    mvwprintw(eqpWin, 4, 6, "Ranged: ");
    mvwprintw(eqpWin, 5, 7, "Armor: ");
    mvwprintw(eqpWin, 6, 6, "Helmet: ");
    mvwprintw(eqpWin, 7, 7, "Cloak: ");
    mvwprintw(eqpWin, 8, 6, "Gloves: ");
    mvwprintw(eqpWin, 9, 7, "Boots: ");
    mvwprintw(eqpWin, 10, 6, "Amulet: ");
    mvwprintw(eqpWin, 11, 7, "Light: ");
    mvwprintw(eqpWin, 12, 6, "Ring R: ");
    mvwprintw(eqpWin, 13, 6, "Ring L: ");

    for (int i = 0; i < 12; i++) {
        if (d->PC->equipment[i]) {
            mvwprintw(eqpWin, 2 + i, 14, "%s", d->PC->equipment[i]->get_name());
        } else {
            mvwprintw(eqpWin, 2 + i, 14, "None");
        }
    }
    mvwprintw(eqpWin, 18, 12, "ESC to exit...");
    wrefresh(eqpWin);
    while (getch() != 27); //Wait for esc key
    delwin(eqpWin);
    io_display(d);
...
{
    WINDOW *prmptWin = newwin(16, 40, 1, 20);
    wclear(prmptWin);
    box(prmptWin, '|', '-');
    mvwprintw(prmptWin, 1, 10, "Enter a number( 0 - 9 )");
    for (int i = 0; i < PC_MAX_INV; i++) {
        if (d->PC->inventory[i]) {
            mvwprintw(prmptWin, 2 + i, 4, "%d. %s", i, d->PC->inventory[i]->get_name());
        } else {
            mvwprintw(prmptWin, 2 + i, 4, "~~Empty~~");
        }
    }
    mvwprintw(prmptWin, 14, 12, "ESC to exit...");
    wrefresh(prmptWin);
    int key = -1;
    while (key != 27)
    {
...
int promptUserForEqpSlot(dungeon *d)
{
    ...
    wclear(prmptWin);
    box(prmptWin, '|', '-');
    mvwprintw(prmptWin, 1, 10, "Enter a number( 0 - 9 or -, =)"); // The document was unclear about a - l, as there are more options than that, so numbers and -, = will do.
    mvwprintw(prmptWin, 2, 9, "0. Weapon: ");
    mvwprintw(prmptWin, 3, 8, "1. Offhand: ");
    mvwprintw(prmptWin, 4, 9, "2. Ranged: ");
    mvwprintw(prmptWin, 5, 10, "3. Armor: ");
    mvwprintw(prmptWin, 6, 9, "4. Helmet: ");
    mvwprintw(prmptWin, 7, 10, "5. Cloak: ");
    mvwprintw(prmptWin, 8, 9, "6. Gloves: ");
    mvwprintw(prmptWin, 9, 10, "7. Boots: ");
    mvwprintw(prmptWin, 10, 9, "8. Amulet: ");
    mvwprintw(prmptWin, 11, 10, "9. Light: ");
    mvwprintw(prmptWin, 12, 9, "-. Ring R: ");
    mvwprintw(prmptWin, 13, 9, "=. Ring L: ");

    for (int i = 0; i < 12; i++) {
        if (d->PC->equipment[i]) {
            mvwprintw(prmptWin, 2 + i, 17, "%s", d->PC->equipment[i]->get_name());
        } else {
            mvwprintw(prmptWin, 2 + i, 14, "None");
        }
    }
    mvwprintw(prmptWin, 14, 12, "ESC to exit...");
    wrefresh(prmptWin);
...
{
    WINDOW *descWin = newwin(20, 40, 1, 20);
    wclear(descWin);
    //box(descWin, '|', '-');

    mvwprintw(descWin, 2, 0, d->PC->inventory[num]->getDesc());
    wrefresh(descWin);
    while (getch() != 27); //Wait for esc key
    delwin(descWin);
    io_display(d);
}
char* get_monster_desc(dungeon *d, const char* name)
{
    for(int i = 0; i < (int)d->monster_descriptions.size(); i++)
    {
        if(!strcmp(name, d->monster_descriptions.at(i).get_name()))
        {
            return d->monster_descriptions.at(i).get_desc();
        }
    }
    return (char *)"Desc not found?";
}
int look_at_monster(dungeon *d)
{
    ...
    bool look = false;
    while (!look) {
        //io_queue_message("Pick monster to look at.");
        io_display(d);
        ...
        switch (getch()) {
            case 't':
                ...
                if(d->character_map[dest[dim_y]][dest[dim_x]] && (dest[dim_x] != d->PC->position[dim_x] && dest[dim_y] != d->PC->position[dim_y]))
                {
                    WINDOW *descWin = newwin(20, 40, 1, 20);
                    wclear(descWin);
                    mvwprintw(descWin, 2, 0, get_monster_desc(d, d->character_map[dest[dim_y]][dest[dim_x]]->name));
                    wrefresh(descWin);
                    while (getch() != 27); //Wait for esc key
                    delwin(descWin);
                    io_display(d);
