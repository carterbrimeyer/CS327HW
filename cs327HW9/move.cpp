...
void do_combat(dungeon *d, character *atk, character *def) {
    ...
    if(def == d->PC)
    {
        io_queue_message("The %s strikes you!", atk->name);
    }
    else
    {
        io_queue_message("You strike the %s!", def->name);
    }
    ...
    if (!def->alive) {//DIE ROUTINE
        ...
        if(strcmp(def->name, "Spongebob Squarepants"))
        {
            d->num_monsters = 0;
        }
    }
...
uint32_t move_pc(dungeon *d, uint32_t dir) {
    ...
    if ((dir != '>') && (dir != '<') && (mappair(next) >= ter_floor)) {
        ...
        {
            pc_pickup_obj(d->PC, d->objmap[next[dim_y]][next[dim_x]]);
