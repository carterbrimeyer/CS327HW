...
void do_combat(dungeon *d, character *atk, character *def) {
    ...
    if (atk == d->PC) {
        ...
        d->monster_descriptions.at(def->descID).inDungeon = false;
    }
    ...
    if (atk != d->PC && def != d->PC) {
        ...
        if (can_see_def && !can_see_atk) {
            io_queue_message("Something kills the helpless %s.",
                             def->name.c_str());
            d->monster_descriptions.at(def->descID).inDungeon = false;
        }
        if (can_see_atk && can_see_def) {
...
void do_moves(dungeon *d) {
    ...
    if (pc_is_alive(d)) {
        ...
        if (d->is_new) {
            ...
            e->time = d->time + (1000 / d->PC->speed.roll());
        }
        e->sequence = 0;
        e->c = d->PC;
    ...
    while (pc_is_alive(d) &&
        ...
        heap_insert(&d->events, update_event(d, e, 1000 / c->speed.roll()));
    }
