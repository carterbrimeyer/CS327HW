...
void io_init_terminal(void) {
    ...
    init_pair(COLOR_YELLOW, COLOR_YELLOW, COLOR_BLACK);   // YELLA
        ...
        for (x = 0; x < 80; x++) {
            ...
            if (d->character_map[y][x] &&
                can_see(d,
                        ...
                        1, 0)) {
                if(d->PC->position[dim_x] == (int16_t)x && d->PC->position[dim_y] == (int16_t)y)
                {
                    ...
                    attron(COLOR_PAIR(d->character_map[y][x]->color.front()));
                    mvaddch(y + 1, x,
                            character_get_symbol(d->character_map[y][x]));
                    attroff(COLOR_PAIR(d->character_map[y][x]->color.front()));
                }
                visible_monsters++;
            }
            else if(d->object_map[y][x] && can_see(d, character_get_pos(d->PC), d->object_map[y][x]->position, 1, 0)){
                attron(COLOR_PAIR(d->object_descriptions.at(d->object_map[y][x]->descID).color));
                mvaddch(y + 1, x, d->object_map[y][x]->getSymbol());
                attroff(COLOR_PAIR(d->object_descriptions.at(d->object_map[y][x]->descID).color));
            }
            else {
...
static const char *adjectives[] = {
        ...
        "A horrendous ",
        "An unnerving ",
        "A little ",    /* Even though they're trying to kill you, */
        "A tiny ",      /* they can still be cute!                 */
        "A fuzzy ",
        "A fluffy ",
...
static void io_list_monsters_display(dungeon *d,
    ...
    for (i = 0; i < count; i++) {
        snprintf(s[i], 40, "%10s%s: %2d %s by %2d %s",
        ...
        if (count <= 13) {
            ...
            mvprintw(i + 6, 19, " %-46s ", s[i]);
        }
    }

    if (count <= 13) {
        mvprintw(count + 6, 19, " %-46s ", "");
        mvprintw(count + 7, 19, " %-46s ", "Hit escape to continue.");
