...
int getUnusedMonsterNum(dungeon *d)
{
    int tmp = rand_range(0, d->monster_descriptions.size() - 1);
    int cnt = 0;
    while(d->monster_descriptions.at(tmp).inDungeon || rand_range(0, 99) >= (int)d->monster_descriptions.at(tmp).rarity)
    {
        tmp = rand_range(0, d->monster_descriptions.size() - 1);
        if(cnt > 5)
        {
            return -1;
        }
...
void gen_monsters(dungeon *d) {
    ...
    d->num_monsters = rand_range(5, 20);
    if (d->num_monsters > d->monster_descriptions.size() - 1) {
        d->num_monsters = d->monster_descriptions.size() - 1;
    }
    for (i = 0; i < d->num_monsters; i++) {
        ...
        int tmp = getUnusedMonsterNum(d);
        if(tmp == -1)
        {
            break;
        }
        d->monster_descriptions.at(tmp).inDungeon = true;
        m = d->monster_descriptions.at(tmp).getAsNPC();
        m->descID = tmp;
    ...
    for(i = 0; i < DUNGEON_Y; i++)
    {
        ...
        {
            if(d->PC->position[dim_y] != (int16_t) i && d->PC->position[dim_x] != (int16_t) j && d->character_map[i][j] && d->character_map[i][j]->alive)
            {
                d->monster_descriptions.at(d->character_map[i][j]->descID).inDungeon = false;
            }
        }
    }
}
