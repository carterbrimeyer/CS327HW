...
*/
int getUnusedObjectNum(dungeon *d)
{
    int tmp = rand_range(0, d->object_descriptions.size() - 1);
    int cnt = 0;
    while(d->object_descriptions.at(tmp).inDungeon || rand_range(0, 99) >= (int)d->object_descriptions.at(tmp).rarity)
    {
        tmp = rand_range(0, d->object_descriptions.size() - 1);
        if(cnt > 5)
        {
            return -1;
        }
...
void gen_objects(dungeon *d) {
    ...
    d->num_objects = rand_range(11, 20);
    if (d->num_objects > d->object_descriptions.size() - 1) {
        d->num_objects = d->object_descriptions.size() - 1;
    }
    for (i = 0; i < d->num_objects; i++) {
        ...
        int tmp = getUnusedObjectNum(d);
        if(tmp == -1)
        {
            break;
        }
        d->object_descriptions.at(tmp).inDungeon = true;
        m = d->object_descriptions.at(tmp).getAsObj();
        m->descID = tmp;
