...
static void dijkstra_corridor(dungeon_t *d, pair_t from, pair_t to)
{
  ...
  while ((p = reinterpret_cast<corridor_path_t *>(heap_remove_min(&h)))) {
    p->hn = NULL;

    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      for (x = to[dim_x], y = to[dim_y];
           (x != (u_int32_t)from[dim_x]) || (y != (u_int32_t)from[dim_y]);
...
int read_rooms(dungeon_t *d, FILE *f)
{
  ...
  for (i = 0; i < d->num_rooms; i++) {
    ...
    for (y = d->rooms[i].position[dim_y];
         (u_int8_t) y < d->rooms[i].position[dim_y] + d->rooms[i].size[dim_y];
         y++) {
      for (x = d->rooms[i].position[dim_x];
           (u_int8_t) x < d->rooms[i].position[dim_x] + d->rooms[i].size[dim_x];
           x++) {
