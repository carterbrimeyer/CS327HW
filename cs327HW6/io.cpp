...
void io_display(dungeon_t *d) {
    ...
    for (y = 0; y < 21; y++) {
        for (x = 0; x < 80; x++) {
            if (d->haveSeen[y][x]) {
                if (d->character[y][x] && (can_see(d, d->character[d->pc.position[dim_y]][d->pc.position[dim_x]], d->character[y][x]))) {
...
uint32_t io_teleport_pc(dungeon_t *d) {
    ...
    bool move = false;
    while (!move) {
        io_queue_message("Pick location to teleport.");
        io_see_all(d);
    ...
    do {
        switch (key = getch()) {
            ...
            case 'L':
                ...
                break;
            case 'g':
                /* Teleport the PC to a selectable place in the dungeon.              */
                io_teleport_pc(d);
                fail_code = 1;
                break;
            case 'm':
